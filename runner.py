...
def runner_generate_task():
    ...
    for all_core in ALL_CORES:
        for efficiency in EFFICIENCYS:
            ...
            output = {
                "tasks":[{'util':t.execution/t.period,'execution':t.execution,'period':t.period,'deadline':t.deadline,'type':'soft' if t.soft else 'hard'} for t in tasks]
            }
...
def phase1():
    for all_core in ALL_CORES:
        for efficiency in EFFICIENCYS:
            ...
            for t in all_task:
                if t["type"]=='soft':
                    task = Task(t["execution"], t["period"], t["deadline"], True)
                    soft_tasks.append(task)
                else:
                    task = Task(t["execution"], t["period"], t["deadline"], False)
